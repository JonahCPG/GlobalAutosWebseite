...
function App() {
  ...
  useEffect(() => {
    const updateGridSize = () => {
      const columns = Math.max(1, Math.ceil(window.innerWidth / GRID_CELL_SIZE))
  ...
  const onGridPointerMove = (event) => {
    ...
    const column = Math.min(columns - 1, Math.max(0, Math.floor((x / bounds.width) * columns)))
  ...
  const cells = useMemo(() => {
    ...
    return Array.from({ length: totalCells }, (_, index) => {
      ...
      const intensity = age < TRAIL_DURATION_MS ? 1 - age / TRAIL_DURATION_MS : 0
  ...
  return (
    <main className="page">
      ...
      <section className="contact-card" aria-label={text.contactAria}>
        ...
        <a className="contact-email" href="mailto:contact@globaleautos.de">
          contact@globaleautos.de
        </a>
      </section>
    </main>
  )
